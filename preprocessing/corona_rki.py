...
tsdateformat = '%Y/%m/%d 00:00:00+00'
outdatetimeformat = '%Y-%m-%d'


def get_county_data(geojson):
    logging.info('Parsing administrative hierarchy')

    Landkreis = namedtuple('Landkreis', ['name', 'AdmUnitId', 'population', 'geojson'])

    geojsons = json.load(geojson)['features']

    lks = list()
    for g in geojsons:
        gj = dict(type="Feature", properties=dict(), geometry=g['geometry'])
        name = g['properties']['county']
        id_ = g['properties']['AdmUnitId']
        pop = g['properties']['EWZ']

        lks.append(Landkreis(name, id_, pop, gj))
...
def load_csv(f, counties):
    logging.info('Loading source data from %s', f.name)

    data = list(csv.DictReader(f))

    timeseries = set()
    for datum in data:
        d = datetime.strptime(datum['Datum'], tsdateformat)
    ...
    for county in counties:
        ...
        geom = asShape(county.geojson['geometry']).centroid
        lat, lng = geom.y, geom.x

        ldata = parse_location_data(timeseries, county, list(filter(lambda x: str(x['AdmUnitId']) == str(county.AdmUnitId), data)))
    ...
    return timeseries_spec, locdata
...
def parse_location_data(ts, loc, data):
    simplified_data = []
    for t in ts:
        t_fmted = datetime.strptime(t, outdatetimeformat).strftime(tsdateformat)
        x = list(filter(lambda datum: datum['Datum'] == t_fmted, data))
        if len(x) == 1:
            total = int(x[0]['KumFall'])
            active = int(x[0]['AnzFallErkrankung'])
            simplified_data.append([
                total,
                total / loc.population * 1000000,
                active,
                active / loc.population * 1000000,
                ])
        elif len(x) == 0:
            simplified_data.append([None, None, None, None])
        else:
            raise UserWarning(F'More than one match for {t_fmted} {loc.AdmUnitId}')
...
def create_metadata():
    ...
    return dict(
        ...
        copyright = "Robert Koch-Institut (RKI), dl-de/by-2-0",
        datasource = "Data from the open domain, retrieved from https://npgeo-corona-npgeo-de.hub.arcgis.com/",
...
if __name__ == '__main__':
    ...
    parser.add_argument('geojson', metavar='<County GeoJSON data>', help='RKI GeoJSON with German counties', type=argparse.FileType('r', encoding='UTF-8'))
    parser.add_argument('history', metavar='<County data>', help='RKI county history CSV', type=argparse.FileType('r', encoding='UTF-8'))
    parser.add_argument('out', metavar='<output file>', help='Output JSON filename', type=argparse.FileType('wb'))

    parsed = parser.parse_args(sys.argv[1:])

    counties = get_county_data(parsed.geojson)
    timeseries, agg = load_csv(parsed.history, counties)

    projs = create_projections(agg, tslen=len(timeseries.series))
    ...
    logging.info('Done processing RKI Corona dataset')
